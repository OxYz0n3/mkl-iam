...
import { BadRequestError, HTTPError, UniqueError } from "../../lib/error";
...
export class AuthService {
    ...
    static async login(body: LoginBody, accessJwt: any, refreshJwt: any, userAgent?: string): Promise<{ user: User, accessToken: string, refreshToken: string }> {
        try {
            ...
            const [ session ] = await db.insert(table.sessions).values({
                ...
                userId: user.id,
                userAgent: userAgent?.slice(0, 255) ?? "Unknown",
            }).returning();
